...
router.get('/affiliates/:id', function(request, response, next) {
  ...
  //Query SalesForce
  const query = 'SELECT Account.Name, Account.Id, Account.Logo__c, Account.Page_Path__c, Account.Website, Account.Languages__c, Account.Summary__c, (SELECT Contact.Name,Contact.Facilitator_For__c,Contact.Photograph__c,Contact.Biography__c FROM Contacts WHERE Contact.Facilitator_For__c != null) FROM Account WHERE RecordType.Name=\'Licensed Affiliate\' AND Account.Name =' + '\'' + request.params.id + '\''
  conn.query(query, function(err, res) {
    if (err) { return console.error(err); }
